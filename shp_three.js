...
// input shp.js defined shpobject. spherize is bool. return THREE.Object3D
p.createModel = function(shp, spherize) {
  var polys = [];
  var lines = [];
  // each record represent one geometry: polygon or polyline
  for (var i=0; i<shp.records.length; i++) {
    ...
    if (r.type === SHP.POLYLINE || r.type === SHP.POLYGON) {
      ...
      var poly = [];
      // for each part of One geometry.
      for (var k=0; k<parts.length; k++) {
        poly = [];
        for (var j=parts[k], last=parts[k+1]||(points.length/2); j<last; j++) {
          var x = points[j*2];
          var y = points[j*2+1];
          // if True. spherize the x, y
          if (spherize) {
            var a = -x/180*Math.PI;
            var t = y/180*Math.PI;
            y = Math.sin(t) * 90;
            x = Math.cos(a) * 90 * Math.cos(t);
            // calc the z-value of points. data is spherize!!!
            var z = Math.sin(a) * 90 * Math.cos(t);
            poly.push(new THREE.Vector3(x, y, z));
          } else {
            // if display data in a plain status
            poly.push(new THREE.Vector3(x, y, 0));
          }
        }
        // no spherized data.. but may be displayed with camera angle!!
  ...
  // data structure of THREE .Obj3D - Line/Mesh { Geometry, Material }
